...
describe("store.data_components", () =>
{
    ...
    function set_up_store(options?: {error?: Error, data?: DBDataComponentRow[]})
    {
        ...
        const range_stub = sinon.stub().callsFake(async () => {
            ...
            return {
                data: options?.data || [mock_db_data_component_row],
                error: null,
            }
        })
    ...
    describe("request_data_component is called", () =>
    {
        ...
        it("should set status to 'loading' when a data component is requested, supabase was called and data was loaded", async () =>
        {
            ...
            const { data_components: data_components2 } = store.getState()
            expect(data_components2.data_component_by_id_and_maybe_version["-123"]!.status).equals("requested", "Async data component status before the Supabase request is made")

            await wait_for(0)
            const { data_components: data_components3 } = store.getState()
            expect(data_components3.data_component_by_id_and_maybe_version["-123"]!.status).equals("loading", "Async data component status after the Supabase request is made but before it resolves")
            ...
            deep_equals(data_components4.data_component_by_id_and_maybe_version["-123"]!.component, expected_data_component, "Data component should be added to the store after loading")
            deep_equals(data_components4.data_component_by_id_and_maybe_version["-123"]!.status, "loaded", "Async data component status after the Supabase request resolves successfully")
            deep_equals(data_components4.data_component_by_id_and_maybe_version["-123v2"]!.component, expected_data_component, "Data component of id and version should be added to the store after loading")
        })
        ...
        it("should set status to 'error' when a data component is requested, supabase was called and an error occurred", async () =>
        {
    ...
    describe("request_data_components is called", () =>
    {
